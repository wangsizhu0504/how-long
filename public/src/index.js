...
function generate(options) {
    const opts = { ...options_1.defaultOptions, ...options };
    const { dotColor, backgroundColor, highlightWeekend, weekendColor, layout } = opts;
    const dotSize = parseInt(opts.dotSize, 10);
    ...
    for (let i = 0; i < 12; i++) {
        const monthDays = (0, util_1.getDaysInMonth)(yyyy, i + 1);
        totalDays += monthDays;
        for (let d = 0; d < monthDays; d++) {
            const offset = monthDays === 31 ? 0 : monthDays === 30 ? dotSize : dotSize * 3;
            const weekend = highlightWeekend && (0, util_1.isWeekend)(new Date(`${yyyy}-${i + 1}-${d + 1}`));
            ...
            if (i + 1 < mm || (i + 1 === mm && d + 1 <= dd)) {
                ...
                dots_elapsed.push(`<circle cx="${cx}" cy="${cy}" ${fill} class="${className}" style="${inlineStyle}" />`);
            }
        }
    }
    const headerYear = yyyy.toString();
    const headerDate = `${mm.toString()}/${dd.toString()}`;
    ...
    const header = isHorizontal
      ...
      <text class="text-year" transform="translate(0, ${padding + dotSize})">${headerYear}/${headerDate}</text>
    ...
    <text class="text-year" transform="translate(${padding}, 0)">${headerYear}/${headerDate}</text>
